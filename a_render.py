...
class PresentationScene(MovingCameraScene):
    ...
    def pause(self, text="---"):
        if not FINAL:
            pause_title = Text(f"{text}", color=C_RED)
            pause_title.scale(0.5).to_corner(LEFT + DOWN)
            pause_title.set_z_index(100)
            self.add(pause_title)
            self.fix(pause_title)

        self.hold(0.15)

        if not FINAL:
            self.remove(pause_title)
    ...
    def pseudo_uniform_points(self, nr=32, dist=0.5, iterations=100, seed=0, disk=False):
        np.random.seed(seed)
        ...
        for _ in range(iterations):
            ...
            for i in range(nr):
                ...
                if not disk:
                    nps[i] = np.array([
                        min(max(nps[i][c], -1.0), 1.0)
                        for c in range(2)
                    ])
                else:
                    nps[i] /= max(np.linalg.norm(nps[i]), 1.0)
            ps = nps
    ...
    def animate_slide_problem_summary(self):
        self.pause("Start problem_summary")

        text_so_eq = Text("Solving Equations", color=C_BLACK).scale(1.6)
        text_so_eq.move_to(ORIGIN)
        text_so_eq.set_z_index(5)
        self.play(
            FadeIn(text_so_eq, shift=UP * 0.5),
            run_time=0.4
        )
        self.pause("Solve equations")

        text_sub = Text("Linear Equations on Spatial Domains", color=C_GRAY).scale(0.8)
        text_sub.next_to(text_so_eq, DOWN)
        self.play(
            FadeIn(text_sub, shift=UP * 0.5),
            run_time=0.4
        )
        self.pause("Show subtext")

        plane = self.load_image("plane").scale_to_fit_height(5.4)
        plane.move_to(UP * 0.4)
        self.play(
            text_so_eq.animate.scale(0.8).to_edge(DOWN),
            FadeOut(text_sub),
            run_time=0.4
        )
        self.play(
        ...
        armadillo = self.load_image("armadillo").scale_to_fit_width(14.0)
        armadillo.move_to(UP * 0.4)
        self.play(
            FadeOut(plane, shift=DOWN),
            FadeIn(armadillo, shift=DOWN),
            run_time=0.6
        )
        self.pause("Show armadillo")

        dragon = self.load_image("dragon").scale_to_fit_height(2.4)
        dromedaris = self.load_image("dromedaris").scale_to_fit_height(2.4).next_to(dragon, DOWN)
        whoosh = self.load_image("whoosh").scale_to_fit_height(4.8).next_to(Group(dragon, dromedaris), RIGHT)
        dancing_group = Group(dragon, dromedaris, whoosh)
        dancing_group.move_to(UP * 0.4)
        self.play(
            FadeOut(armadillo, shift=DOWN),
            FadeIn(dancing_group, shift=DOWN),
            run_time=0.6
        )
        self.pause("Show dancing")

        self.play(
            FadeOut(dancing_group),
            text_so_eq.animate.to_edge(UP),
            run_time=0.6
        )
        ...
        texts_solve = []
        for name in methods:
            text = Text(name, color=C_DARK_GRAY).scale(0.8)
            if texts_solve:
                text.align_to(texts_solve[-1], UP).shift(DOWN * 0.72)
            texts_solve.append(text)
        Group(*texts_solve).move_to(ORIGIN)

        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=DOWN * 0.5) for obj in texts_solve],
                lag_ratio=0.2
            ),
            run_time=0.8
        )
        self.pause("Show different solving options")

        text_new = Text(name, color=C_BLACK, weight=BOLD).scale(0.8)
        text_new.move_to(texts_solve[-1])
        self.play(
            *[obj.animate.set_color(C_LIGHT_GRAY) for obj in texts_solve[:-1]],
            *[Transform(obj_a, obj_b) for obj_a, obj_b in zip(texts_solve[-1], text_new)],
            run_time=0.4
        )
        self.pause("Highlight multigrid")

        self.play(
            *[FadeOut(obj) for obj in texts_solve],
            FadeOut(text_so_eq),
            run_time=0.4
        )

        radius = 3.2
        x_axis = Arrow(color=C_DARK_GRAY).put_start_and_end_on(np.array([-radius, -radius, 0.0]), np.array([radius, -radius, 0.0])).set_cap_style(CapStyleType.ROUND)
        x_axis.set_z_index(10)
        y_axis = Arrow(color=C_DARK_GRAY).put_start_and_end_on(np.array([-radius, -radius, 0.0]), np.array([-radius, radius, 0.0])).set_cap_style(CapStyleType.ROUND)
        y_axis.set_z_index(10)
        x_label = Text("Time to Solve", color=C_DARK_GRAY).scale(0.5)
        x_label.set_y(x_axis.get_y()).align_to(x_axis, RIGHT).shift(DOWN * 0.36)
        x_label.set_z_index(10)
        y_label = Text("Time to Construct", color=C_DARK_GRAY).scale(0.5).rotate(np.pi / 2)
        y_label.set_x(y_axis.get_x()).align_to(y_axis, UP).shift(LEFT * 0.36)
        y_label.set_z_index(10)
        self.play(
            self.create_arrow(x_axis),
            self.create_arrow(y_axis),
            run_time=0.4
        )
        self.play(
            FadeIn(x_label),
        ...
        self.pause("Show graph")

        data = [
            ([(0, 0), (0, 1), (1, 0)], C_GREEN),
            ([(0, 3), (3, 0), (2, 0), (0, 2)], C_ORANGE),
            ([(1, 3), (3, 3), (3, 1)], C_RED),
        ]
        polygons = []
        for ps, color in data:
            ps = np.array([[x - 1.5, y - 1.5, 0.0] for x, y in ps]) * (2 * radius / 4)
            color = lerp(C_WHITE, color, 0.25)

            polygon = Polygon(*ps).set_stroke(color, width=96).set_fill(color, opacity=1).round_corners(0.02)
        ...
        self.pause("Show region")

        dots = []
        np.random.seed(1)
        for mu, sigma in [
            (np.array([-0.6, 0.4, 0.0]) * radius, 0.8),
            (np.array([0.4, -0.6, 0.0]) * radius, 0.8),
        ]:
            dots_new = []
            for _ in range(8):
                dot = Circle(0.08).set_fill(C_BLACK, opacity=1).set_stroke(opacity=0)
                pos = mu + np.array([
                ...
                dot.move_to(pos)
                dots_new.append(dot)
            dots.extend(dots_new)

            self.play(
                AnimationGroup(
                    *[FadeIn(obj, scale=3) for obj in dots_new],
                    lag_ratio=0.5
                ),
                run_time=0.6
            )
            self.pause(f"Show dots around ({mu[0]:.2f}, {mu[1]:.2f})")

        dot_gravo = Circle(0.08).set_fill(C_BLACK, opacity=1).set_stroke(opacity=0)
        dot_gravo.move_to(np.array([-0.5, -0.5, 0.0]) * radius)
        dots.append(dot_gravo)

        self.play(
            FadeIn(dot_gravo, scale=5),
            run_time=0.6
        )
        self.pause("Show Gravo dot")

        paper_wiersma_2023 = self.load_image("paper_wiersma_2023")
        paper_wiersma_2023.scale_to_fit_height(10.0)
        paper_wiersma_2023.to_edge(UP).shift(DOWN * 1.2)
        paper_wiersma_2023.set_z_index(51)
        paper_outline = Rectangle(C_LIGHT_GRAY, paper_wiersma_2023.height, paper_wiersma_2023.width).set_stroke(width=4)
        paper_outline.move_to(paper_wiersma_2023)
        paper_outline.set_z_index(52)
        paper_shadow = Rectangle(C_BLACK, paper_wiersma_2023.height, 0.24).set_stroke(opacity=0).set_fill(opacity=0.25)
        paper_shadow.move_to(paper_wiersma_2023.get_right() + (paper_shadow.width / 2) * RIGHT + DOWN * 0.12)
        paper_shadow.set_z_index(50)
        paper_group = Group(
            paper_wiersma_2023,
            paper_outline,
            paper_shadow
        ...
        self.play(
            FadeIn(paper_group, shift=UP * 2),
            run_time=0.6
        )
        self.pause("Show paper")
    ...
    def animate_slide_dirichlet_demonstration(self):
        ...
        walls[0] = Polygon(
            ...
            np.array([-b, -b, 0]),
            np.array([-a, -a, 0]),
        ).set_fill(C_LIGHT_GRAY, opacity=1).set_stroke(opacity=0)
        walls[1] = walls[0].copy().rotate(np.pi / 2, about_point=ORIGIN)
        walls[2] = walls[1].copy().rotate(np.pi / 2, about_point=ORIGIN)
        walls[3] = walls[2].copy().rotate(np.pi / 2, about_point=ORIGIN)
        self.play(
        ...
        wall_texs = [None] * 4
        wall_texs[0] = MathTex(f"{val_to_deg(1.0):.1f}\\small{{^{{\\circ}}\\text{{C}}}}", color=lerp(C_BLACK, C_ORANGE, 0.5)).scale(0.8).next_to(walls[0], LEFT).shift(RIGHT * 0.1)
        wall_texs[1] = MathTex(f"{val_to_deg(0.0):.1f}\\small{{^{{\\circ}}\\text{{C}}}}", color=lerp(C_BLACK, C_LIGHT_GRAY, 0.5)).scale(0.8).next_to(walls[1], DOWN).shift(UP * 0.1)
        wall_texs[2] = MathTex(f"{val_to_deg(-1.0):.1f}\\small{{^{{\\circ}}\\text{{C}}}}", color=lerp(C_BLACK, C_BLUE, 0.5)).scale(0.8).next_to(walls[2], RIGHT).shift(LEFT * 0.1)
        wall_texs[3] = MathTex(f"{val_to_deg(1.0):.1f}\\small{{^{{\\circ}}\\text{{C}}}}", color=lerp(C_BLACK, C_ORANGE, 0.5)).scale(0.8).next_to(walls[3], UP).shift(DOWN * 0.1)
        self.play(
            walls[0].animate.set_fill(C_ORANGE),
            walls[2].animate.set_fill(C_BLUE),
            walls[3].animate.set_fill(C_ORANGE),
            FadeIn(wall_texs[0], shift=LEFT),
            FadeIn(wall_texs[1], shift=DOWN),
            FadeIn(wall_texs[2], shift=RIGHT),
            FadeIn(wall_texs[3], shift=UP),
            run_time=0.4
        )
        self.pause("Show wall temperatures")
        ...
        camera_shift_4 = np.array([0.0, 0.6, 0.0])
        camera_zoom_4 = 0.8
        self.play(
            first_gs_animation,
            self.camera.frame.animate.shift((amount - 1) * LEFT * spacing).shift(-camera_shift_3).scale(camera_zoom_3).scale(1 / camera_zoom_4).shift(camera_shift_4),
        ...
        for iy in range(n // 2):
            for ix in range(n // 2):
                ...
                cell_hot_big = cell_big.copy().set_fill(C_ORANGE)
                cell_hot_big.set_z_index(cell_hot_big.get_z_index() + 1)
                cells_hot_big[iy][ix] = cell_hot_big
                cells_hot_big_flat.append(cell_hot_big)

                cell_cold_big = cell_big.copy().set_fill(C_BLUE)
                cell_cold_big.set_z_index(cell_cold_big.get_z_index() + 1)
                cells_cold_big[iy][ix] = cell_cold_big
                cells_cold_big_flat.append(cell_cold_big)
        ...
        camera_shift_5 = np.array([4.4, 0.6, 0.0])
        camera_zoom_5 = 0.8
        ...
        self.play(
            ...
            *animations,
            self.camera.frame.animate.shift(-camera_shift_4).scale(camera_zoom_4).scale(1 / camera_zoom_5).shift(camera_shift_5),
            run_time=1.2
        )
    ...
    def animate_slide_multigrid_diagram(self):
        ...
        for i in range(3):
            icons[i].move_to(np.array([-5.0, spacing_vert * (1 - i), 0.0]))

        timeline_0 = Line(LEFT * 4.2, RIGHT * 20.0).set_stroke(C_LIGHT_GRAY, width=8).set_cap_style(CapStyleType.ROUND).set_y(icons[0].get_y())
        ...
        for i in range(gs_number):
            ...
            gs_icon.shift(UP * spacing_vert + RIGHT * (i * (gs_icon.get_width() + gap) - 4.0))
        ...
        restrict_01_text = Text("Restrict", color=C_RED).scale(0.5).rotate(angle).set_opacity(0)
        ...
        double_arrow = DoubleArrow(color=C_GRAY).put_start_and_end_on(
            np.array([-6.2, icons[0].get_y(), 0.0]),
            np.array([-6.2, icons[1].get_y(), 0.0])
        )
        finer_text = Text("Fine", color=C_GRAY).scale(0.5).next_to(double_arrow, UP).shift(DOWN * 0.1)
        coarser_text = Text("Coarse", color=C_GRAY).scale(0.5).next_to(double_arrow, DOWN).shift(UP * 0.1)
        ...
        prolong_10_text = Text("Prolong", color=C_RED).scale(0.5).rotate(-angle).set_opacity(0)
        ...
        self.play(
            double_arrow.animate.put_start_and_end_on(double_arrow.get_start(), double_arrow.get_end() + spacing_vert * DOWN),
            coarser_text.animate.shift(spacing_vert * DOWN),
            FadeIn(icons[2], scale=0.8),
            run_time=0.6
        )
        self.pause("Get new domain")
        ...
        self.play(
            restrict_01_text.animate.set_opacity(1),
            restrict_12_text.animate.set_opacity(1),
            prolong_21_text.animate.set_opacity(1),
            prolong_10_text.animate.set_opacity(1),
            run_time=0.4
        )
        self.pause("Write Restrict and Prolong")
    ...
    def animate_slide_gravo_demonstration(self):
        ...
        title_tex.scale(1.6).next_to(paper_wiersma_2023, UP)
        self.play(
            FadeIn(title_tex),
            run_time=0.6,
            rate_func=rush_from
        ...
        for i in range(E.shape[0]):
            edge = Line(V[E[i][0]], V[E[i][1]]).set_stroke(C_LIGHT_GRAY, width=8).set_cap_style(CapStyleType.BUTT)
            edge.set_z_index(6)
        ...
        self.pause("Show step: Prolongating")
    ...
    def animate_slide_tetrahedral_meshes(self):
        ...
        emoji_1_text = self.load_image("emoji_tools").scale(0.25)
        ...
        emoji_2_text = self.load_image("emoji_trend").scale(0.25)
        ...
        emoji_3_text = self.load_image("emoji_timer").scale(0.25)
    ...
    def animate_slide_contribution_basics(self):
        ...
        self.play(
            FadeOut(code_diagram[4][0][1], scale=0.0),
            FadeIn(tetrahedralizing_text, scale=0.0),
            run_time=0.6
        )
        ...
        self.pause("Highlight \"Prolongating\"")
        ...
        white_rectangle = Rectangle(C_WHITE, 100, 100).set_fill(opacity=0.9)
        white_rectangle.set_z_index(50)
        spot_mg_0 = self.load_image("spot_tet_mg_raw_0")
        spot_mg_0.scale_to_fit_height(6.0)
        spot_mg_0.move_to(LEFT * 3.0)
        spot_mg_0.set_z_index(60)
        spot_mg_1 = self.load_image("spot_tet_mg_raw_1")
        spot_mg_1.scale_to_fit_height(6.0)
        spot_mg_1.move_to(RIGHT * 3.0)
        spot_mg_1.set_z_index(60)
        arrow_mg = Arrow(spot_mg_0.get_right(), spot_mg_1.get_left(), color=C_DARK_GRAY)
        arrow_mg.set_z_index(60)
        Group(spot_mg_0, arrow_mg, spot_mg_1).shift(DOWN * 0.3)
        self.play(
            FadeIn(white_rectangle),
            FadeIn(spot_mg_0, shift=UP),
            run_time=0.6
        )
        self.pause("Show fine spot")

        self.play(
            FadeIn(arrow_mg, shift=UP),
            FadeIn(spot_mg_1, shift=UP),
            run_time=0.6
        )
        self.pause("Show coarse spot")

        self.clear(run_time=0.6)

    def animate_slide_experiments(self):
        title_tex = self.create_title("Experiments")
        title_tex.generate_target()
        title_tex.scale(2.0).move_to(ORIGIN)
        self.play(
            FadeIn(title_tex, shift=UP),
        ...
        equation_tex = MathTex("\\bigl|\\bigl|", "A \\mathbf{x}", "=", "\\mathbf{b}", "\\bigl|\\bigl|", color=C_BLACK).scale(2)
        equation_tex.set_color_by_tex("\\bigl|\\bigl|", C_WHITE)
        self.play(
            Write(equation_tex),
            run_time=0.4
        )
        ...
        norm_tex = MathTex("\\bigl|\\bigl|", "A \\mathbf{x}", "-", "\\mathbf{b}", "\\bigl|\\bigl|", color=C_BLACK).scale(2)
        norm_tex.set_color_by_tex("\\bigl|\\bigl|", C_WHITE)
        norm_tex.set_z_index(5)
        self.play(
            TransformMatchingTex(equation_tex, norm_tex),
            run_time=0.4
        )
        self.pause("Change to minus")

        self.play(
            norm_tex[0].animate.set_color(C_BLACK),
            norm_tex[4].animate.set_color(C_BLACK),
            run_time=0.4
        )
        self.pause("Turn into residual norm")

        example_plots = []
        for it in range(1, 21):
            img = self.load_image(f"plot_conv_example_{it}")
            img.scale_to_fit_height(6.0)
            example_plots.append(img)
        self.play(
            FadeIn(example_plots[0]),
            norm_tex.animate.scale(0.3).rotate(np.pi / 2).next_to(example_plots[0], LEFT).shift(np.array([0.3, 0.16, 0.0])),
            run_time=0.6
        )
        self.play(
            FadeOut(norm_tex),
        ...
        self.pause("Show example plot")

        self.play(
            FadeIn(example_plots[1]),
            run_time=0.4
        )
        self.pause("1 V-cycle")

        self.play(
            FadeIn(example_plots[2]),
            run_time=0.4
        )
        self.pause("2 V-cycle")

        for i in range(3, len(example_plots)):
            self.add(example_plots[i])
            self.hold(0.05)
        self.remove(*example_plots[:-1])
        self.pause("All V-cycles")

        containers = []
        crosses = []
        for i in range(3):
            cont = Rectangle(C_LIGHT_GRAY, 5.4, 3.2).set_stroke(width=6).round_corners(0.05)
            cont.shift(np.array([4.0 * (i - 1), -0.4, 0.0]))
            containers.append(cont)

            if i:
                x = (containers[i - 1].get_x() + containers[i].get_x()) / 2
                cross = MathTex("\\times", color=C_DARK_GRAY).move_to(containers[i]).set_x(x)
        ...
        for i, name in enumerate(title_names):
            title = Text(name, weight=BOLD, color=C_BLACK)
            title.next_to(containers[i], DOWN).scale(0.75).shift(UP * containers[i].height)
            titles.append(title)

        alg_names = [
            ("AMG-RS", "Ruge-St\\\"uben AMG", C_RED),
            ("AMG-SA", "Smoothed Aggreg. AMG", C_ORANGE),
            ("SHI06", "Graph-Based GMG", C_GREEN),
            ("GRAVO", "Our Approach", C_BLUE)
        ]
        alg_texts = []
        alg_desc_texts = []
        for i, (name, desc, color) in enumerate(alg_names):
            text = MathTex(f"\\texttt{{\\textbf{{{name}}}}}", color=color)
            text.scale(1.0).move_to(titles[0]).shift(DOWN * (0.8 + 0.9 * i))
            alg_texts.append(text)

            desc_text = MathTex(f"\\textbf{{{desc}}}", color=lerp(C_BLACK, color, 0.5))
            desc_text.scale(0.48).move_to(text).shift(DOWN * 0.36)
            alg_desc_texts.append(desc_text)
        self.play(
            FadeOut(example_plots[-1]),
            FadeIn(containers[0], shift=UP),
            FadeIn(titles[0], shift=UP),
            run_time=0.4
        )
        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=DOWN) for obj in alg_texts],
                lag_ratio=0.5
            ),
            AnimationGroup(
                *[FadeIn(obj, shift=DOWN) for obj in alg_desc_texts],
                lag_ratio=0.5
            ),
            run_time=0.6
        )
        self.pause("Show different algorithms")

        # paper_shi_2006 = self.load_image("paper_shi_2006")
        # paper_shi_2006.scale_to_fit_height(8.0)
        # paper_shi_2006.to_edge(UP).shift(np.array([2.4, -1.2, 0.0]))
        # paper_shi_2006.set_z_index(51)
        # paper_outline = Rectangle(C_LIGHT_GRAY, paper_shi_2006.height, paper_shi_2006.width).set_stroke(width=4)
        # paper_outline.move_to(paper_shi_2006)
        # paper_outline.set_z_index(52)
        # paper_shadow = Rectangle(C_BLACK, paper_shi_2006.height, 0.24).set_stroke(opacity=0).set_fill(opacity=0.25)
        # paper_shadow.move_to(paper_shi_2006.get_right() + (paper_shadow.width / 2) * RIGHT + DOWN * 0.12)
        # paper_shadow.set_z_index(50)
        # paper_group = Group(
        #     paper_shi_2006,
        #     paper_outline,
        ...
        for i, name in enumerate(mesh_names):
            text = MathTex(f"\\texttt{{\\textbf{{{name.replace('_', '\\_')}}}}}", color=C_DARK_GRAY)
            text.scale(0.8).move_to(titles[1]).shift(DOWN * (0.8 + 0.5 * i))
            mesh_texts.append(text)

            if name != "...":
                img = self.load_image(f"mesh_{name}")
                img.scale_to_fit_height(2.4)
                img.move_to(np.array([3.2 + 2.4 * (i % 2), 2.4 - 1.2 * i, 0.0]))
                mesh_imgs.append(img)
        self.play(
            # FadeOut(paper_group),
            FadeIn(containers[1], shift=UP),
            FadeIn(titles[1], shift=UP),
            FadeIn(crosses[0], shift=UP),
            run_time=0.4
        )
        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=DOWN) for obj in mesh_texts],
                lag_ratio=0.5
            ),
            *[FadeIn(obj) for obj in mesh_imgs],
            run_time=0.6
        )
        self.pause("Show different meshes")
        ...
        for i, name in enumerate(system_names):
            ...
            text_1 = MathTex(f"\\texttt{{\\textbf{{{name_1}}}}}", color=C_DARK_GRAY)
            text_1.scale(0.8).move_to(titles[2]).shift(DOWN * (0.8 + 1.0 * i))
            text_2 = MathTex(f"\\texttt{{\\textbf{{{name_2}}}}}", color=C_DARK_GRAY)
            text_2.scale(0.8).move_to(titles[2]).shift(DOWN * (1.2 + 1.0 * i))
            system_texts.append(Group(text_1, text_2))
        self.play(
            FadeIn(containers[2], shift=UP),
            FadeIn(titles[2], shift=UP),
            FadeIn(crosses[1], shift=UP),
            *[FadeOut(obj) for obj in mesh_imgs],
            run_time=0.4
        )
        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=DOWN) for obj in system_texts],
                lag_ratio=0.5
            ),
            run_time=0.6
        )
        self.pause("Show different systems")
        ...
        for i, name in enumerate(conv_names):
            ...
            img = self.load_image(f"plot_conv_{name}_a")
            img.scale_to_fit_height(3.9)
            img.shift(np.array([img.width * x, -(img.height + 0.1) * y, 0.0]))
            img.set_z_index(60)
            conv_group.add(img)
        conv_group.move_to(ORIGIN)

        covers = []
        for i, img in enumerate(conv_group):
            cover = Rectangle(C_WHITE, img.height, img.width).set_stroke(opacity=0).set_fill(opacity=0)
            cover.move_to(conv_group[i])
            cover.set_z_index(61)
            covers.append(cover)
        self.add(*covers)

        self.unfix(title_tex)
        self.play(
            *[
            ...
            title_tex.animate.shift(UP * 2),
            FadeIn(conv_group),
            *[FadeIn(obj) for obj in covers],
            run_time=0.6
        )
        self.pause("Show a couple of plots")
        ...
        for focus in [
            ...
            {8, 9},
            {6, 7, 8, 9},
            {*range(len(conv_group))}
        ]:
            self.play(
                *[covers[idx].animate.set_fill(opacity=0.75 * (idx not in focus)) for idx in range(len(conv_group))],
                run_time=0.4
            )
            self.pause(f"Focus on {', '.join(str(idx) for idx in focus)}")
        ...
        white_rectangle = Rectangle(C_WHITE, 100, 100).set_fill(opacity=0.9)
        white_rectangle.set_z_index(90)
        spot_mg_1 = self.load_image("spot_tet_mg_raw_1")
        spot_mg_1.scale_to_fit_height(7.0)
        spot_mg_1.set_z_index(95)
        self.play(
            FadeIn(white_rectangle),
            FadeIn(spot_mg_1, shift=UP),
            run_time=0.6
        )
        self.pause("Show bad spot")

        self.clear(run_time=0.6)

    def animate_slide_boundary(self):
        title_tex = self.create_title("Boundary")

        cube_boundaries = [None] * 8
        for i in range(len(cube_boundaries)):
            cube_boundary = self.load_image(f"cube_boundary_{i}")
            cube_boundary.scale_to_fit_height(6.0)
            cube_boundary.set_z_index(25)
            cube_boundaries[i] = cube_boundary
        self.play(
            FadeIn(title_tex),
            FadeIn(cube_boundaries[0]),
            run_time=0.6
        )
        self.fix(title_tex)
        ...
        self.play(
            FadeIn(cube_boundaries[1]),
            run_time=0.4
        )
        self.remove(cube_boundaries[0])
        self.pause("Show surface points")
        ...
        for i, (s, c) in enumerate(order_texts_data):
            ...
            text.to_edge(LEFT).set_y(i * 1.0 + (i == 2) * -0.06)
            text.set_z_index(30)
            rank_texts.append(text)
        Group(*rank_texts).next_to(cube_boundaries[1], RIGHT).shift(2 * LEFT)

        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=2 * RIGHT) for obj in rank_texts[:2]],
                lag_ratio=0.5
            ),
            cube_boundaries[1].animate.shift(2 * LEFT),
            run_time=0.6
        )
        self.pause("Show labels")

        for b in range(2, 4):
            cube_boundaries[b].move_to(cube_boundaries[b - 1])
            self.play(
                FadeIn(rank_texts[b], shift=2 * RIGHT),
                FadeIn(cube_boundaries[b]),
                run_time=0.4
            )
            self.remove(cube_boundaries[b - 1])
            self.pause(f"Show points of rank {b}")

        emoji_programmer = self.load_image("emoji_programmer").scale_to_fit_height(2)
        emoji_programmer.next_to(cube_boundaries[3], LEFT).shift(0.5 * RIGHT)
        scale_group = Group(
            cube_boundaries[3],
            *rank_texts
        )
        ...
        scale_group.target.scale(scale).shift(2 * RIGHT)
        arrows = [
            Arrow(emoji_programmer.get_right(), scale_group.target.get_left(), color=C_DARK_GRAY),
            None,
            None
        ]
        self.play(
            MoveToTarget(scale_group),
            FadeIn(emoji_programmer, shift=2 * RIGHT),
            FadeIn(arrows[0], shift=2 * RIGHT),
            run_time=0.6
        )
        self.pause("Show programmer")

        emoji_draw = self.load_image("emoji_draw").scale_to_fit_height(0.8)
        emoji_draw.next_to(arrows[0], UP).shift(np.array([-0.6, -0.24, 0.0]))
        slash_text = Text("/", color=C_DARK_GRAY).scale_to_fit_height(0.8)
        slash_text.next_to(arrows[0], UP).shift(np.array([0.0, -0.24, 0.0]))
        emoji_gear = self.load_image("emoji_gear").scale_to_fit_height(0.8)
        emoji_gear.next_to(arrows[0], UP).shift(np.array([0.6, -0.3, 0.0]))

        self.play(
            FadeIn(emoji_draw, shift=UP * 0.5),
            run_time=0.4
        )
        self.pause("Show draw emoji")

        self.play(
            FadeIn(slash_text),
            FadeIn(emoji_gear, shift=UP * 0.5),
            run_time=0.4
        )
        self.pause("Show gear emoji")

        for i in range(2):
            cube_boundaries[4 + i].scale(scale).move_to(cube_boundaries[3 + i]).shift(np.array([4.8, -2.4, 0.0]))
            cube_boundaries[6 + i].scale(scale).move_to(cube_boundaries[4 + i])

            start = cube_boundaries[3 + i].get_bottom()
            end = start + np.array([2.4, -1.2, 0.0])
            arrows[1 + i] = CurvedArrow(start, end, color=C_DARK_GRAY, angle=1.0)

        camera_zoom_1 = 0.7
        camera_shift_1 = np.array([3.0, -1.8, 0.0])
        self.play(
            self.camera.frame.animate.scale(1 / camera_zoom_1).shift(camera_shift_1),
            *[FadeOut(obj) for obj in rank_texts],
            *[FadeIn(obj) for obj in cube_boundaries[4:6]],
            *[FadeIn(obj) for obj in arrows[1:]],
            run_time=0.6
        )
        self.pause("Show hierarchy")

        emojis_gear = [emoji_gear, None, None]
        for i in range(2):
            emojis_gear[1 + i] = emoji_gear.copy().align_to(arrows[1 + i], LEFT + DOWN).shift(np.array([0.1, -0.4, 0.0]))
        self.play(
            [FadeIn(obj, shift=0.5 * np.array([-1.0, -2.0, 0.0]) / np.sqrt(5)) for obj in emojis_gear[1:]],
            run_time=0.4
        )
        self.pause("Show new gears")

        white_rectangle = Rectangle(C_WHITE, 100, 100).set_fill(opacity=0.9)
        white_rectangle.set_z_index(50)
        emoji_gear = emojis_gear[0].copy().scale_to_fit_height(1.8)
        emoji_gear.move_to(camera_shift_1).shift(UP * 4.0)
        emoji_gear.set_z_index(60)
        self.play(
            FadeIn(white_rectangle),
            FadeIn(emoji_gear),
            run_time=0.4
        )
        self.pause("Jump into algorithm")
        ...
        ps = sorted(ps, key=lambda p: p[0] - p[1] * 5)
        boundary_fine = [int(p[0] > 0.95) + int(p[1] > 0.95) for p in ps]

        boundary_colors = [C_GRAY, C_BLUE, C_GREEN]
        vertices_fine = []
        for i, (x, y) in enumerate(ps):
            vertex = Circle(0.12).set_fill(boundary_colors[boundary_fine[i]], opacity=1).set_stroke(opacity=0)
            vertex.move_to((x, y, 0))
            vertex.set_z_index(55)
            vertices_fine.append(vertex)
        Group(*vertices_fine).scale_to_fit_height(5.0).scale(1 / camera_zoom_1).shift(camera_shift_1).shift(np.array([-5.0, -0.6, 0.0]))

        edges_fine = []
        for i in range(nr):
            for j in range(i + 1, nr):
                if np.linalg.norm(ps[i] - ps[j]) < dist * 1.05:
                    edge = Line(vertices_fine[i].get_center(), vertices_fine[j].get_center()).set_stroke(C_LIGHT_GRAY, width=16).set_cap_style(CapStyleType.ROUND)
                    edge.set_z_index(54)
                    edges_fine.append(edge)
        ...
        for i, indices in enumerate(clusters):
            color = lerp(C_WHITE, C_GRAY, 0.25)

            vs = [vertices_fine[j].get_center() for j in indices]
            polygon = Polygon(*vs).round_corners(0.04)
            polygon.set_stroke(color, width=128).set_fill(color, opacity=1)
            polygon.set_z_index(53)
            cluster_polygons.append(polygon)

        self.play(
            *[FadeIn(obj) for obj in vertices_fine],
            *[FadeIn(obj) for obj in edges_fine],
            *[FadeIn(obj) for obj in cluster_polygons],
            run_time=0.6
        )
        ...
        self.pause("Make fine domain appear")
        ...
        for cluster in clusters:
            x, y = ps[cluster[0]]
            vertex = Circle(0.18).set_fill(C_GRAY, opacity=1).set_stroke(opacity=0)
            vertex.move_to((x, y, 0))
            vertex.set_z_index(55)
            vertices_coarse.append(vertex)
        Group(*vertices_coarse).scale_to_fit_height(5.0).scale(1 / camera_zoom_1).shift(camera_shift_1).shift(np.array([5.0, -0.6, 0.0]))

        edges_coarse = []
        for i in range(len(vertices_coarse)):
            for j in range(i + 1, len(vertices_coarse)):
                if i + j != 4:
                    edge = Line(vertices_coarse[i].get_center(), vertices_coarse[j].get_center()).set_stroke(C_LIGHT_GRAY, width=24).set_cap_style(CapStyleType.ROUND)
                    edge.set_z_index(54)
                    edges_coarse.append(edge)
        Line().set_cap_style(CapStyleType.BUTT)

        arrow_mg = Arrow(ORIGIN, RIGHT * 2.0, color=C_DARK_GRAY)
        arrow_mg.move_to(camera_shift_1)
        arrow_mg.set_z_index(60)
        self.play(
            *[FadeIn(obj) for obj in vertices_coarse],
            *[FadeIn(obj) for obj in edges_coarse],
            FadeIn(arrow_mg),
            run_time=0.6
        )
        ...
        if not FINAL:
            for i in range(len(vertices_coarse)):
                ...
                debug_indices.append(debug_index)
        self.add(*debug_indices)
        self.pause("Make coarse domain appear")
        self.remove(*debug_indices)

        boundary_coarse = [max(boundary_fine[idx] for idx in cluster) for cluster in clusters]
        for b in range(1, 3):
            color = boundary_colors[b]
            mid_color = lerp(C_WHITE, color, 0.4)
            light_color = lerp(C_WHITE, color, 0.25)
            scale = 1.2

            self.play(
                *[obj.animate.set_stroke(mid_color).set_fill(mid_color) for obj, b_obj in zip(cluster_polygons, boundary_coarse) if b <= b_obj],
                run_time=0.4
            )
            self.play(
                *[obj.animate.set_stroke(light_color).set_fill(light_color) for obj, b_obj in zip(cluster_polygons, boundary_coarse) if b <= b_obj],
                run_time=0.4
            )
            if b == 1:
                self.pause(f"Highlight rank {b} clusters")

            self.play(
                *[obj.animate.set_fill(color).scale(scale) for obj, b_obj in zip(vertices_coarse, boundary_coarse) if b <= b_obj],
                run_time=0.4
            )
            self.play(
                *[obj.animate.scale(1 / scale) for obj, b_obj in zip(vertices_coarse, boundary_coarse) if b <= b_obj],
                run_time=0.4
            )
            self.pause(f"Propagate rank {b}")

        self.play(
            *[FadeOut(obj) for obj in vertices_fine],
            *[FadeOut(obj) for obj in edges_fine],
            *[FadeOut(obj) for obj in cluster_polygons],
            *[FadeOut(obj) for obj in vertices_coarse],
            *[FadeOut(obj) for obj in edges_coarse],
            FadeOut(emoji_gear),
            FadeOut(white_rectangle),
            FadeOut(arrow_mg),
            run_time=0.4
        )
        self.pause("Oh there goes gravity")

        scale = 1.1
        for i in range(2):
            cube_boundaries[6 + i].set_opacity(0)
            self.play(
                cube_boundaries[4 + i].animate.scale(scale),
                cube_boundaries[6 + i].animate.scale(scale).set_opacity(1),
                run_time=0.24
            )
            self.remove(cube_boundaries[4 + i])
            self.play(
                cube_boundaries[6 + i].animate.scale(1 / scale),
                run_time=0.24
            )
        self.pause("Propagate boundary")

        self.clear(run_time=0.6)

    def animate_slide_technique_1_sampling_density(self):
        title_tex = self.create_title("Technique 1: Sampling Density")
        for obj in title_tex:
            obj.save_state()

        title_tex_2 = self.create_title("Technique 2: Sampling Order")
        title_tex_3 = self.create_title("Technique 3: Pit Prevention")
        title_tex_4 = self.create_title("Technique 4: Boundary-Aware Smoothing")
        ...
        for i in range(1, len(title_group)):
            title_group[i].next_to(title_group[i - 1], DOWN).align_to(title_group[i - 1], LEFT).shift(DOWN * 0.2)
        title_group.move_to(ORIGIN)

        self.play(
            AnimationGroup(
                *[FadeIn(obj, shift=0.5 * UP) for obj in title_group],
                lag_ratio=0.2
            ),
            run_time=0.6
        )
        self.pause("Show optimizations")

        self.play(
            *[obj.animate.restore() for obj in title_tex],
            FadeOut(title_tex_2),
            FadeOut(title_tex_3),
            FadeOut(title_tex_4),
            run_time=0.6
        ...
        radius = 3.0
        for x, y in ps:
            vertex_fine = Circle(0.06).set_fill(C_GRAY, opacity=1).set_stroke(opacity=0)
            vertex_fine.move_to((x, y, 0))
            vertex_fine.set_z_index(5)
            vertices_fine.append(vertex_fine)
        Group(*vertices_fine).scale_to_fit_height(2 * radius).move_to(center)
        ...
        boundary = [np.linalg.norm(p) > 0.95 for p in ps]
        self.play(
            *[obj.animate.set_fill(C_BLUE) for obj, b in zip(vertices_fine, boundary) if b],
            *[Flash(obj, color=C_BLUE, flash_radius=0.08, line_stroke_width=5, z_index=50) for obj, b in zip(vertices_fine, boundary) if b],
            run_time=0.6
        )
        self.pause("Flash boundary")

        edges_fine_to_keep = []
        edges_fine_to_remove = []
        for e in edges_fine:
            if boundary[e.start_index] or boundary[e.end_index]:
                edges_fine_to_remove.append(e)
            else:
                edges_fine_to_keep.append(e)
        edges_fine = edges_fine_to_keep

        self.play(
            *[Uncreate(obj) for obj in edges_fine_to_remove],
            run_time=0.4
        )

        vertices_boundary = [obj for obj, b in zip(vertices_fine, boundary) if b]
        vertices_boundary = sorted(vertices_boundary, key=lambda obj: np.atan2(obj.get_y(), -obj.get_x()))

        nr_boundary = sum(boundary)
        vertices_fine_new = []
        angle = 2 * np.pi / (3 * nr_boundary)
        animations = []
        for i in range(1, nr_boundary, 2):
            vertex_fine = vertices_boundary[i].copy()
            vertex_fine.set_z_index(5)
            vertices_fine_new.append(vertex_fine)

            animations.extend([
                Rotate(vertices_boundary[i], -angle, about_point=center),
                Rotate(vertex_fine, angle, about_point=center),
            ])
        vertices_fine.extend(vertices_fine_new)
        vertices_boundary.extend(vertices_fine_new)
        boundary.extend([1] * len(vertices_fine_new))
        self.play(
            *animations,
            run_time=0.8
        )
        ...
        edges_fine_new = []
        for i in range(len(vertices_fine)):
            for j in range(i + 1, len(vertices_fine)):
                bs = int(boundary[i]) + int(boundary[j])
                if bs == 1 and np.linalg.norm(vertices_fine[i].get_center() - vertices_fine[j].get_center()) < radius * dist * 0.95:
                    edge_fine = Line(vertices_fine[i].get_center(), vertices_fine[j].get_center()).set_stroke(C_LIGHT_GRAY, width=12).set_cap_style(CapStyleType.ROUND)
                    edge_fine.set_z_index(4)
                    edges_fine_new.append(edge_fine)
        for i in range(len(vertices_boundary)):
            edge_fine = Line(vertices_boundary[i - 1].get_center(), vertices_boundary[i].get_center()).set_stroke(C_LIGHT_GRAY, width=12).set_cap_style(CapStyleType.ROUND)
            edge_fine.set_z_index(4)
            edges_fine_new.append(edge_fine)

        self.play(
            *[Create(obj) for obj in edges_fine_new],
            run_time=0.4
        )
        self.pause("Increase boundary")

        region_outer = Arc(radius - 0.15, 0, 2 * np.pi, color=lerp(C_WHITE, C_GREEN, 0.5), stroke_width=64).set_cap_style(CapStyleType.ROUND)
        region_outer.move_to(center)
        region_inner = Circle(radius - 0.55, color=lerp(C_WHITE, C_RED, 0.5)).set_stroke(opacity=0).set_fill(opacity=1)
        region_inner.move_to(center)
        ...
        self.hold(0.4)
        self.play(
            region_outer.animate.set_stroke(lerp(C_WHITE, C_GREEN, 0.35)),
            run_time=0.6
        )
        self.pause("Highlight outer region")

        self.play(
            FadeIn(region_inner, scale=0.5),
            run_time=0.4
        )
        self.hold(0.4)
        self.play(
            region_inner.animate.set_fill(lerp(C_WHITE, C_RED, 0.35)),
            run_time=0.6
        )
        self.pause("Highlight inner region")

        self.clear(run_time=0.6)

    def animate_slide_technique_2_sampling_order(self):
        title_tex = self.create_title("Technique 2: Sampling Order")
        self.play(
            FadeIn(title_tex),
            run_time=0.6
        )
        ...
        for it in range(2):
            ...
            if it == 1:
                ...
                self.play(
                    ...
                    *[Flash(obj, color=boundary_colors[b], flash_radius=0.16, line_stroke_width=5, z_index=50) for obj, b in zip(vertices_fine, boundary) if b],
                    run_time=0.6
                )
                self.pause("Flash boundary")
    ...
    def animate_slide_technique_3_pit_prevention(self):
        ...
        self.play(
            ...
            FadeIn(arrow, shift=UP * 0.5),
            FadeIn(image_after, shift=UP * 0.5),
            run_time=0.6
        )
        self.pause("Remind of pits")

        title_tex = self.create_title("Technique 3: Pit Prevention")
        self.play(
            FadeIn(title_tex),
            run_time=0.6
        )
        ...
        for i in range(V.shape[0]):
            ...
            vertex.set_z_index(10)
            vertices_fine.append(vertex)
        Group(*vertices_fine).scale(scale).move_to(np.array([0.0, -0.5, 0.0]))
        ...
        edges_boundary = sorted(sorted(edges_fine, key=lambda obj: -obj.get_y())[:3], key=lambda obj: obj.get_x()) \
            + [max(edges_fine, key=lambda obj: obj.get_x())]

        animations = []
        for i in range(len(edges_boundary)):
            animations.append(
                edges_boundary[i].animate.set_stroke(C_BLUE, width=edges_boundary[i].get_stroke_width() * 1.35)
            )

            if i < len(vertices_boundary):
                animations.append(
                    vertices_boundary[i].animate.scale(1.35).set_fill(C_GREEN if i == 2 else C_BLUE)
                )
        ...
        self.play(
            ...
            *[obj.animate.set_stroke(C_LIGHT_GRAY, width=obj.get_stroke_width() / 1.35) for obj in edges_boundary],
            run_time=0.4
        )
        self.pause("Highlight boundary")

        to_shift_group = Group(*vertices_fine, *edges_fine)
        self.play(
            code_diagram.animate.shift(RIGHT * 4),
            to_shift_group.animate.shift(RIGHT * 1.2),
            run_time=0.6
        )
        self.pause("Show code diagram")

        vertices_fine[0].save_state()
        self.play(
            code_diagram[0][0][0][0].animate.set_stroke(C_ORANGE).set_fill(lerp(C_WHITE, C_ORANGE, 0.5)),
            vertices_fine[0].animate.set_stroke(opacity=0).set_fill(C_LIGHT_GRAY),
            *[obj.animate.set_fill(lerp(C_BLACK, obj.get_fill_color(), 0.5)).scale(1.35) for obj in vertices_fine[1:4]],
            run_time=0.4
        )
        self.pause("Sample points")

        cluster_colors = [C_GRAY, C_BLUE, C_GREEN]
        in_cluster = [-1, 0, 1, 2, 1, 1, 2, 0, 0]
        ...
        for i, indices in enumerate([
            ...
            is_line = len(indices) == 2
            color = lerp(C_WHITE, cluster_colors[i], 0.25)
            gradient = [color] * 2 + [C_WHITE] * (2 + (i == 1))

            vs = [vertices_fine[j].get_center() for j in indices]
            polygon = (Line if is_line else Polygon)(*vs).set_cap_style(CapStyleType.ROUND)
            if not is_line:
                polygon.round_corners(0.04)
            polygon.set_stroke(gradient, width=128).set_fill(gradient, opacity=1)
            ...
            if is_line:
                ...
                e12 = vs[2] - vs[1]
                alpha = np.dot(vs[0] - vs[1], e12) / np.dot(e12, e12)
                v_proj = vs[1] + alpha * e12
                direction = v_proj - vs[0]
            polygon.set_sheen_direction(direction)
        ...
        self.play(
            vertices_fine[0].animate.restore(),
            code_diagram[0][0][0][0].animate.set_stroke(C_LIGHT_GRAY).set_fill(lerp(C_WHITE, C_LIGHT_GRAY, 0.5)),
            code_diagram[1][0][0][0].animate.set_stroke(C_ORANGE).set_fill(lerp(C_WHITE, C_ORANGE, 0.5)),
            *[MoveToTarget(obj) for group in cluster_polygons for obj in group],
            *[vertices_fine[j + 1].animate.set_fill(c) for j, c in enumerate(cluster_colors)],
            run_time=0.4
        )
        ...
        for it in range(2):
            if it:
                gradient_vert = [C_BLUE] * 2 + [C_GRAY] * 2

                arrow_up = Arrow(stroke_width=12).put_start_and_end_on(
                    vertices_fine[1].get_center() + DOWN * 0.1,
                    vertices_fine[0].get_center() + UP * 0.1,
                ).set_stroke(gradient_vert).set_fill(C_BLUE).set_sheen_direction(DOWN)
                arrow_up.shift(RIGHT * 0.4)
                ...
                cross.move_to(arrow_up).shift(RIGHT * 0.4)
                ...
                checkmarks = []
                for i in range(2):
                    start = vertices_fine[2 + i].get_center()
                    end = vertices_fine[0].get_center()
                    ...
                    if i == 1:
                        perp *= -1
                    mid = (start + end) / 2
                    length = arrow_up.get_length()

                    color = [C_BLUE, C_GREEN][i]
                    gradient_side = [color] * 2 + [C_BLUE] * 2

                    arrow_side = Arrow(max_stroke_width_to_length_ratio=100).put_start_and_end_on(
                        mid - para * (length / 2),
                        mid + para * (length / 2)
                    ).set_stroke(gradient_side).set_fill(C_BLUE).set_sheen_direction(para)
                    arrow_side.shift(perp * 0.4)
            ...
            cluster_extra = Line(start - diff * 0.05, start + diff).set_cap_style(CapStyleType.ROUND)
            ...
            self.pause("Move to connecting step")
            ...
            hole_polygon = Polygon(*Vc).set_stroke(opacity=0.0).set_fill([C_RED, C_ORANGE][it], opacity=0.5)
    ...
    def animate_slide_technique_4_boundary_aware_smoothing(self):
        title_tex = self.create_title("Technique 4: Boundary-Aware Smoothing")
        self.play(
            FadeIn(title_tex),
            run_time=0.6
        )
        ...
        for it in range(2):
            ...
            if it == 1:
                ...
                self.pause("Remove some small molds")
        ...
        mg_images = [None] * 3
        mg_arrows = [None] * 2
        for i, name in enumerate(["0", "all_1", "ba_1"]):
    ...
    def animate_slide_experiment_revised(self):
        container = Rectangle(C_LIGHT_GRAY, 5.8, 3.2).set_stroke(width=6).round_corners(0.05)

        title = Text("Algorithms", weight=BOLD, color=C_BLACK)
        title.next_to(container, DOWN).scale(0.75).shift(UP * container.height)
        ...
        alg_names = [
            ...
            ("SHI06", "Graph-Based GMG", C_GREEN),
            ("GRAVO", "Our Approach", C_BLUE),
            ("GRAVO++", "Preserves Boundary", C_PURPLE)
        ]
        alg_texts = []
        alg_desc_texts = []
        for i, (name, desc, color) in enumerate(alg_names):
            text = MathTex(f"\\texttt{{\\textbf{{{name}}}}}", color=color)
            text.scale(1.0).move_to(title).shift(DOWN * (0.8 + 0.9 * i))
            alg_texts.append(text)
            ...
            alg_desc_texts.append(desc_text)
        self.play(
            FadeIn(container, shift=UP),
            FadeIn(title, shift=UP),
            *[FadeIn(obj, shift=UP) for obj in alg_texts[:-1]],
            *[FadeIn(obj, shift=UP) for obj in alg_desc_texts[:-1]],
            run_time=0.4
        )
        self.pause("Show old algorithms list")

        new = self.load_image("new").scale_to_fit_height(0.6)
        new.move_to(alg_texts[-1].get_corner(UP + RIGHT)).shift(RIGHT * 0.3)
        self.play(
            FadeIn(alg_texts[-1], shift=DOWN),
            FadeIn(alg_desc_texts[-1], shift=DOWN),
            run_time=0.4
        )
        self.play(
            FadeIn(new, scale=1.5),
            run_time=0.4
        )
        self.pause("Add GRAVO++")
        ...
        for i, name in enumerate(conv_names):
            ...
            img = self.load_image(f"plot_conv_{name}_b")
            img.scale_to_fit_height(3.9)
            img.shift(np.array([img.width * x, -(img.height + 0.1) * y, 0.0]))
            img.set_z_index(60)
            conv_group.add(img)
        ...
        self.play(
            ...
            FadeOut(title),
            FadeOut(new),
            *[FadeOut(obj) for obj in alg_texts],
            *[FadeOut(obj) for obj in alg_desc_texts],
            run_time=0.4
        )
        self.pause("Show new plots")
        ...
        for i, name in enumerate(time_names):
            ...
            img = self.load_image(f"plot_time_{name}")
            img.scale_to_fit_width(6.5)
            img.shift(np.array([img.width * x, -(img.height + 0.1) * y, 0.0]))
            img.set_z_index(70)
            time_group.add(img)
        ...
        covers = []
        for i, img in enumerate(time_group):
            cover = Rectangle(C_WHITE, img.height, img.width).set_stroke(opacity=0).set_fill(opacity=0)
            cover.move_to(time_group[i])
            cover.set_z_index(71 + (i == 0))
            covers.append(cover)
        self.add(*covers)

        time_group[0].generate_target()
        time_group[0].scale(1.6).move_to(ORIGIN)
        self.play(
            FadeOut(conv_group),
            FadeIn(time_group[0]),
            run_time=0.6
        )
        self.pause("Show time plot")

        self.play(
            *[FadeIn(obj) for obj in time_group[1:]],
            MoveToTarget(time_group[0]),
            run_time=0.6
        )
        self.pause("Show all time plots")

        for focus in [
            {0, 1, 2, 3},
            {4, 5},
            {*range(len(time_group))}
        ]:
            self.play(
                *[covers[idx].animate.set_fill(opacity=0.75 * (idx not in focus)) for idx in range(len(time_group))],
                run_time=0.4
            )
            self.pause(f"Focus on {', '.join(str(idx) for idx in focus)}")
        ...
        for idx, obj in enumerate(buenos):
            ...
            obj.move_to(np.array([3.6 * (2 * idx % 3 - 1), 0.4 * (1 - 2 * (idx // 2)), 0.0]))
        self.play(
    ...
    def animate_slide_summary_conclusion(self):
        spot_tet_mg = [None] * 3
        for i in range(3):
            spot_tet_mg[i] = self.load_image(f"spot_tet_mg_{i}").scale(0.54)
            spot_tet_mg[i].move_to(np.array([5.6 - 1.2 * i, 2.0 * (1 - i), 0.0]))

        contribution_texts = [self.create_title("Our Work")]
        for i, text in enumerate([
            "- An extension of Gravo MG for tetrahedral meshes;",
            "- Four effective techniques for boundary preservation;",
            "- An experimental analysis demonstrating its benefits."
        ]):
            text = Text(text, color=C_DARK_GRAY).scale(0.6).align_to(contribution_texts[-1], LEFT + UP).shift(DOWN * (0.64 + 0.1 * (i == 0)))
            contribution_texts.append(text)

        for i in range(len(contribution_texts)):
            self.play(
                *[FadeIn(obj) for obj in spot_tet_mg] * (i == 0),
                FadeIn(contribution_texts[i], shift=RIGHT * (i != 0)),
                run_time=0.4
            )
            self.pause(f"Show contribution_texts[{i}]")

        future_texts = [self.create_title("Future Work")]
        future_texts[0].align_to(contribution_texts[-1], LEFT + UP).shift(DOWN * 1.0)
        for i, text in enumerate([
            "- Better control on sampling;",
            "- Better/baked-in computation of boundary;",
            "- Robustness against \"bad spots\";",
            "- Experiments in real-life applications.",
        ]):
            text = Text(text, color=C_DARK_GRAY).scale(0.6).align_to(future_texts[-1], LEFT + UP).shift(DOWN * (0.64 + 0.1 * (i == 0)))
            future_texts.append(text)

        for i in range(len(future_texts)):
            self.play(
                FadeIn(future_texts[i], shift=RIGHT),
                run_time=0.4
            )
            self.pause(f"Show future_texts[{i}]")
    ...
    def animate_slide_last_page(self):
        ...
        white_rectangle = Rectangle(C_WHITE, 100, 100).set_fill(opacity=1)
        white_rectangle.set_z_index(50)
        self.play(
            FadeOut(white_rectangle),
            run_time=0.6
        )
        self.pause("Last page")
    ...
    def animate(self):
        ...
        self.animate_slide_problem_summary()
        ...
        self.animate_slide_experiments()

        self.animate_slide_boundary()
        self.animate_slide_technique_1_sampling_density()
        self.animate_slide_technique_2_sampling_order()
        self.animate_slide_technique_3_pit_prevention()
        self.animate_slide_technique_4_boundary_aware_smoothing()
        self.animate_slide_experiment_revised()

        self.animate_slide_summary_conclusion()
        self.animate_slide_last_page()
